...
$persistPath = "$env:ProgramData\PolosAutomation\MyTools\templink1.json"
$null = New-Item -ItemType Directory -Path (Split-Path $persistPath) -Force

# Load existing settings or create new hashtable
if (Test-Path $persistPath) {
    Write-Host "HELLO"
    $json = Get-Content $persistPath
    $settings = ConvertFrom-Json $json | ConvertTo-Hashtable
...
# Update values without overwriting the whole file
if ($setSource) {
    $settings["Source"] = $setSource
}
if ($setDestination) {
    $settings["Destination"] = $setDestination
}

# Save updated settings
$settings | ConvertTo-Json -Depth 3 | Set-Content -Path $persistPath
...
if ($Host.Name -eq 'Windows PowerShell ISE Host') {
    Write-Host "Running in ISE"
} elseif ($Host.Name -like '*ConsoleHost*') {
    Write-Host "Running in terminal (ConsoleHost)"
} else {
    Write-Host "Unknown host: $($Host.Name)"
}
